...
const inputWrapperClassName = cn(
  "relative w-full max-w-[420px] rounded-2xl border border-[#e3e7ec] bg-[#f5f7fa] p-4",
  "has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-[#a855f7]",
  "dark:border-[#2b2a25] dark:bg-[#171716]",
);

const inputClassName =
  "w-full bg-transparent outline-none placeholder:text-[#646b75]/40 dark:placeholder:text-[#9a958a]/40";

type InputFieldProps = ComponentPropsWithoutRef<"input"> & {
  wrapperClassName?: string;
};

type SmoothInputType = "text" | "password" | "email" | "tel" | "number";
...
const SmoothInput = ({
  ...
  const getCaretIndex = (target: HTMLInputElement) => {
    ...
    const dir = target.selectionDirection ?? "forward";
    return dir === "backward" ? selectionStart : selectionEnd;
  };
  ...
  useEffect(() => {
    ...
    const textBeforeCaret = type === "password"
      ? PASSWORD_CHAR.repeat(caretIndex)
      : input.value.slice(0, caretIndex);
    const absWidth = measurePrefixWidth(textBeforeCaret);
    if (absWidth !== null) {
      caretX.set(absWidth);
    }
  }, [type]);
  ...
  return (
    <div className={cn(inputWrapperClassName, wrapperClassName)}>
      ...
      >
        ...
        />
        <motion.div
          className="pointer-events-none col-start-1 col-end-2 row-start-1 row-end-2 h-[0.9em] w-0.5 self-center rounded-full bg-[#18181b] dark:bg-[#fafafa]"
